...
export const setProjects = (state: { projects: Array<IProject> | [] } = initialState, action: ProjectAction) => {
  ...
  switch (action.type) {
    ...
    case ProjectActionTypes.ADD_TODO:
      const project = state.projects.find(project => action.payload.id == project.id)
      return  { ...state,
        projects: state.projects.map(proj => proj.id === project?.id ? {...proj, todoList: [...proj.todoList, action.payload.todo]} : {...proj})
      }
    default:
      return { ...state };
  }
